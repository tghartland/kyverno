	...
	return func(writer http.ResponseWriter, request *http.Request) {
		...
		if admissionReview.Request.Kind.Kind == "Lease" {
			if logger.V(6).Enabled() {
				...
				logger2.V(6).Info("admission review request processed", "time", time.Since(startTime).String())
			}
		} else {
			if logger.V(4).Enabled() {
				...
				logger2.V(4).Info("admission review request processed", "time", time.Since(startTime).String())
			}
		}
	}
}
...
func Verify(m *webhookconfig.Monitor, logger logr.Logger) AdmissionHandler {
	return func(request *admissionv1.AdmissionRequest) *admissionv1.AdmissionResponse {
		if logger.V(6).Enabled() {
			...
			logger2.V(6).Info("incoming request", "last admission request timestamp", m.Time())
		}
